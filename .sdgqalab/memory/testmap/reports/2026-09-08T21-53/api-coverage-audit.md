...
|------|-------|--------------|
| `netlify/functions/api.js` helpers | api | Optional: extract `allowedOrigin` / `allowedFields` / `parseBody` for pure unit tests (taxonomy only — not a production-risk gap) |
